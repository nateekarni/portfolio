...
export default async function handler(request) {
    ...
    if (request.method === 'GET') {
        try {
            const { data, error } = await supabasePublic.from('contact_sections').select('*').limit(1).single();
    ...
    if (request.method === 'PUT') {
        ...
        try {
            ...
            const { data: existing } = await supabaseAdmin.from('contact_sections').select('id').limit(1).single();
            let query = supabaseAdmin.from('contact_sections');
