...
export default async function handler(request) {
    ...
    if (request.method === 'PUT') {
        try {
            ...
            const { data, error } = await supabaseAdmin.from('certifications').update(body).eq('id', id).select().single();
            if (error) return errorResponse('Failed to update cert', 500);
    ...
    if (request.method === 'DELETE') {
        try {
            ...
            if (error) return errorResponse('Failed to delete cert', 500);
